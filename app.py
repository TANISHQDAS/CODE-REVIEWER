...
# Helper to determine writable stats file path
def get_stats_file_path():
    if os.environ.get("VERCEL"):
        return os.path.join("/tmp", STATS_FILE)
    return STATS_FILE

# ===== STATS FUNCTIONS =====
def load_stats():
    """Load stats from JSON file safely"""
    file_path = get_stats_file_path()
    if not os.path.exists(file_path) and os.path.exists(STATS_FILE):
...
def save_stats(stats):
    """Save stats to JSON file, with fallback for read-only serverless filesystems"""
    file_path = get_stats_file_path()
    try:
        with open(file_path, 'w') as f:
            json.dump(stats, f, indent=2)
    except OSError:
        # Prevent 500 errors on read-only file systems like Vercel Serverless
        pass
